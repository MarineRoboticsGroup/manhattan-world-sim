...
class ManhattanWaterworld:
    ...
    def __init__(
        ...
        assert isinstance(cell_scale, int)
        ...
        if robot_area is not None:
            assert len(robot_area) == 2
            assert isinstance(robot_area[0], tuple)
            assert isinstance(robot_area[1], tuple)
            assert len(robot_area[0]) == 2
            assert len(robot_area[1]) == 2

        assert isinstance(landmark_area, list) or landmark_area is None
        if landmark_area is not None:
            assert len(landmark_area) == 2
            assert isinstance(landmark_area[0], tuple)
            assert isinstance(landmark_area[1], tuple)
            assert len(landmark_area[0]) == 2
            assert len(landmark_area[1]) == 2

        # create grid
        self._grid = np.zeros(
            grid_vertices_shape, dtype=np.float32
        )
        ...
        if robot_area is not None:
            ...
            bl, tr = landmark_area
            self._landmark_feasibility = np.zeros(
                (self._x_pts, self._y_pts), dtype=bool
            )
    ...
    def set_robot_area_feasibility(
        self, area: List[tuple], feasibility: Union[bool, int]
    ):
    ...
    def set_landmark_area_feasibility(
        self, area: List[tuple], feasibility: Union[bool, int]
    ):
        mask = np.zeros((self._x_pts, self._y_pts), dtype=bool)
        bl, tr = area
    ...
    def get_neighboring_robot_vertices(
        self, i: int, j: int, feasibility=None
    ) -> List[tuple]:
        nb_pts = self.get_neighboring_vertices(i, j)
        return self.pick_robot_vertices(nb_pts, feasibility)

    def pick_robot_vertices(
        self, vertices, feasibility: np.ndarray = None
    ) -> List[tuple]:
        feasible_pts = []
    ...
    def nearest_robot_vertex_coordinates(
        self, x: float, y: float, feasibility: np.ndarray = None
    ) -> List[tuple]:
    ...
    def add_landmark(self, lmk: BeaconAgent, i: int, j: int):
        # TODO replace lmk with BeaconAgent functionality
        assert isinstance(lmk, BeaconAgent)
    ...
    def add_robot(self, rbt: RobotAgent, i: int, j: int, orientation=0.0):
        # TODO replace rbt functionality with RobotAgent functionality
        assert isinstance(rbt, RobotAgent)
        assert isinstance(i, int) and isinstance(j, int)
...
class ManhattanWorld(ManhattanWaterworld):
    ...
    def __init__(self, grid_vertices_shape: tuple = (9, 9), cell_scale: int = 1):
