...
class ManhattanWaterworldTestCase(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.tr = (4, 3)
        cls.size = (7, 6)
        cls.env1 = ManhattanWorld(cls.size, cls.scale, [cls.bl, cls.tr])
        cls.env2 = ManhattanWorld(
            cls.size, cls.scale, None, [cls.bl, cls.tr], check_collision=False
        )
        ...
        cls.vertices = np.array(
            [np.meshgrid(np.arange(cls.size[0]), np.arange(cls.size[1]))]
        ).T.reshape(-1, 2)
    ...
    def test_init(self):
        ...
        for env in envs:
            ...
            self.assertCountEqual(
                set(map(tuple, self.vertices)), set(map(tuple, env.vertices))
            )
        ...
        self.assertCountEqual(
            self.env1_rbt_vertices, set(map(tuple, self.env1.robot_feasible_vertices))
        )
        self.assertCountEqual(
            self.env2_lmk_vertices,
            set(map(tuple, self.env2.landmark_feasible_vertices)),
        )
        self.assertCountEqual(
            set(map(tuple, self.env1.vertices))
            - set(map(tuple, self.env1.robot_feasible_vertices)),
            set(map(tuple, self.env2.vertices))
            - set(map(tuple, self.env2.landmark_feasible_vertices)),
        )
    ...
    def test_rbt_lmk(self):
        for i, env in enumerate(self.envs):
            if i == 0:
                ...
                self.assertTrue(
                    env.update_robot_pose("rbt1", SE2Pose(6, 6, -np.pi / 2))
                )
                self.assertFalse(
                    env.update_robot_pose("rbt1", SE2Pose(6, 6.6, -np.pi / 2))
                )
                self.assertFalse(
                    env.update_robot_pose("rbt1", SE2Pose(0, 0, -np.pi / 2))
                )
            else:
    ...
    def test_vertices_coordinates(self):
        env = ManhattanWorld(self.size, self.scale, [self.bl, self.tr])
        ...
        self.assertCountEqual(
            env.vertices2coordinates(gt_rbt_v),
        ...
        near_coords = env.nearest_robot_vertex_coordinates(
            *env.vertex2coordinate(*self.bl)
        )
    ...
    def test_set_feasible_area(self):
        ...
        env.set_landmark_area_feasibility(
            [(self.bl[0] - 1, self.bl[1] - 1), (self.tr[0] + 1, self.tr[1] + 1)], False
        )
    ...
    def test_edge_path(self):
        ...
        env = ManhattanWorld((6, 6), 1, [(1, 1), (3, 3)])
        wpts = env.robot_edge_path()
        gt_wpts = [
            (1, 1),
            (2, 1),
